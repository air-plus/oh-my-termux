# shellcheck    shell=dash
. "$___X_CMD_PKG_METADATA_PATH/.x-cmd/pip-populate.sh"
___x_cmd_pkg___pip_populate_install_dep(){
    local bin_dir="$1"

    local x_=
    if x pkg websrc pip_ 2>/dev/null || true; then
        "$___X_CMD_PKG___META_TGT/$bin_dir/pip" install \
            --require-virtualenv                        \
            'httpx[socks]' pytest-playwright            \
            -i "$x_"
        "$___X_CMD_PKG___META_TGT/$bin_dir/playwright" install chromium --with-deps --no-shell
    fi

}

___x_cmd_pkg___pip_populate_install(){
    local pkg_install_name="$___X_CMD_PKG___META_NAME"
    local bin_dir="$1"

    local x_=
    if x pkg websrc pip_ 2>/dev/null || true; then
        "$___X_CMD_PKG___META_TGT/$bin_dir/pip" install \
            --require-virtualenv                                  \
            "$pkg_install_name==${___X_CMD_PKG___META_VERSION#v}" \
            'browser-use[cli]'                                    \
            -i "$x_"
    fi
}

___x_cmd_pkg___pip_populate --python_version v3.11.0+23.11.0-2 browser-use playwright
